...
import os
import threadRecieveData # Import threadRecieveData.py in the same directory
...
class InfoWindow(Screen):
    ...
    def saveSelf(self):
        with open(ROOT_DIR + '/RecieveData/tmp/' + self.filename.text + '.txt', 'w') as f:
...
class AcquireWindow(Screen):
    ...
    def loadSelf(self):
        ...
        with open(ROOT_DIR + '/RecieveData/tmp/' + self.current + '.txt', 'r') as f:
            lines = f.readlines()
        ...
        nameThreadAcquire = Thread(target=threadRecieveData.acquireData, args=(lines[0].rstrip(), int(lines[1].rstrip()), int(lines[2].rstrip()), stopThread))
    ...
    def plotData(self):
        threadRecieveData.plotDataNpz(self.current)
